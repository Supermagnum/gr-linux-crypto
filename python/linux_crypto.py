...
def _aes_encrypt(key: bytes, data: bytes, key_size: int, iv_mode: Union[str, bytes]) -> Tuple[bytes, bytes, None]:
    ...
    if iv_mode == "random":
        ...
        iv = iv_mode.encode() if isinstance(iv_mode, str) and len(iv_mode) == 16 else secrets.token_bytes(16)
