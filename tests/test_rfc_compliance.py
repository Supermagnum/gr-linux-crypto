...
try:
    from test_rfc_vectors import (
        RFC7027Parser, RFC6954Parser, RFC8734Parser,
        RFC7027TestVector, RFC6954TestVector, RFC8734TestVector
    )
except ImportError:
    from .test_rfc_vectors import (
        RFC7027Parser, RFC6954Parser, RFC8734Parser,
    ...
    def test_rfc7027_test_vectors(self, rfc7027_vectors):
        ...
        if rfc7027_vectors:
            for vector in rfc7027_vectors:
                ...
                assert vector.curve_name in crypto.get_brainpool_curves(), \
            ...
            for curve_name in curves:
                ...
                assert len(pub_pem) > 0, f"RFC 7027: Public key serialization failed for {curve_name}"
                assert len(priv_pem) > 0, f"RFC 7027: Private key serialization failed for {curve_name}"
                assert b'BEGIN' in pub_pem, f"RFC 7027: Public key must be PEM format for {curve_name}"
                assert b'BEGIN' in priv_pem, f"RFC 7027: Private key must be PEM format for {curve_name}"
...
class TestRFC6954Compliance:
    ...
    def test_rfc6954_ecdh_operations(self, curve_name):
        ...
        assert initiator_shared == responder_shared, \
        ...
        assert len(initiator_shared) == expected_length, \
...
class TestRFC8734Compliance:
    ...
    def test_rfc8734_ecdh_operations(self, curve_name):
        ...
        assert len(client_shared) == expected_length, \
