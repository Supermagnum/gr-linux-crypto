...
class TestMultiRecipientECIES(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.key_store = CallsignKeyStore(store_path=self.temp_file.name, use_keyring=False)
        self.ecies = MultiRecipientECIES(curve=self.curve, key_store_path=self.temp_file.name)
    ...
    def test_single_recipient(self):
        ...
        decrypted = self.ecies.decrypt(encrypted, callsign, private_key_pem.decode('ascii'))
    ...
    def test_recipient_not_in_list(self):
        ...
        with self.assertRaises(ValueError):
            self.ecies.decrypt(encrypted, excluded_callsign, excluded_private_key_pem.decode('ascii'))
    ...
    def test_different_curves(self):
        ...
        for curve in ["brainpoolP256r1", "brainpoolP384r1", "brainpoolP512r1"]:
            with self.subTest(curve=curve):
                ...
                key_store = CallsignKeyStore(store_path=temp_file.name, use_keyring=False)
    ...
    def test_callsign_case_insensitive(self):
        ...
        callsign, private_key_pem, _ = self.test_keypairs[0]
        decrypted = self.ecies.decrypt(encrypted, callsign.upper(), private_key_pem.decode('ascii'))
