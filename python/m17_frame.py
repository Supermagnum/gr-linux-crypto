...
class M17Frame:
    ...
    @staticmethod
    def create_lsf_frame(src: str, dst: str, meta: bytes = b"",
                        encryption_type: M17EncryptionType = M17EncryptionType.NONE,
                        encryption_subtype: int = 0,
                        key_fingerprint: bytes = b"") -> 'M17Frame':
        """
    ...
    @staticmethod
    def create_stream_frame(payload: bytes, frame_counter: int,
                          encryption_type: M17EncryptionType = M17EncryptionType.NONE,
                          key: Optional[bytes] = None,
                          nonce: Optional[bytes] = None) -> 'M17Frame':
        """
        ...
        try:
            if algorithm == 'chacha20':
                ...
                if self.frame_counter > 0:
                    self.nonce = struct.pack('>Q', self.frame_counter).ljust(12, b'\x00')
                else:
                    import secrets
...
class M17SessionKeyExchange:
    ...
    @staticmethod
    def encrypt_key_for_recipient(session_key: bytes, recipient_key_id: str) -> Optional[bytes]:
        """
