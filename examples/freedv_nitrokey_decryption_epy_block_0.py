...
class blk(gr.sync_block):
    ...
    def work(self, input_items, output_items):
        ...
        if len(self._encrypted_buffer) >= self._frame_size_with_tag and len(self._key_buffer) >= 32:
            try:
                # Get encrypted frame (Codec2 frame + auth tag)
                encrypted_frame = bytes(self._encrypted_buffer[:self._frame_size_with_tag])
                self._encrypted_buffer = self._encrypted_buffer[self._frame_size_with_tag:]
                ...
                nonce = self._nonce_counter.to_bytes(self._nonce_length, byteorder='little')
                ...
                if CRYPTOGRAPHY_AVAILABLE:
                    ...
                    try:
                        decrypted_frame = nacl.decrypt_chacha20_poly1305(encrypted_frame, key, nonce)
