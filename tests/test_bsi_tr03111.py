...
class BSITR03111Validator:
    ...
    @staticmethod
    def validate_key_generation(curve_name: str, private_key: EllipticCurvePrivateKey,
                                public_key: EllipticCurvePublicKey) -> Tuple[bool, List[str]]:
        """
    ...
    @staticmethod
    def validate_ecdh_operation(curve_name: str, shared_secret: bytes) -> Tuple[bool, List[str]]:
        """
        ...
        if len(shared_secret) != expected_length:
            issues.append(f"Shared secret length incorrect: {len(shared_secret)} != {expected_length}")
    ...
    @staticmethod
    def validate_ecdsa_signature(curve_name: str, signature: bytes, 
                                 hash_algorithm: str) -> Tuple[bool, List[str]]:
        """
        ...
        hash_bits = {
            ...
            'sha384': 384,
            'sha512': 512
        }.get(hash_algorithm.lower(), 0)
        
        curve_security = BSITR03111Validator.APPROVED_CURVES.get(curve_name, {}).get('security_level', 0)
        
        # Hash should be at least as strong as curve security level
        if hash_bits < curve_security:
            issues.append(f"Hash algorithm {hash_algorithm} ({hash_bits} bits) "
...
class TestBSITR03111Compliance:
    ...
    def test_curve_parameter_validation(self, curve_name):
        ...
        assert validator.validate_curve_parameters(curve_name), \
    ...
    def test_key_generation_compliance(self, curve_name):
        ...
        for i in range(10):
            ...
            is_valid, issues = validator.validate_key_generation(curve_name, private_key, public_key)
    ...
    def test_ecdh_compliance(self, curve_name):
        ...
        for i in range(10):
            ...
            is_valid, issues = validator.validate_ecdh_operation(curve_name, alice_shared)
            assert is_valid, \
    ...
    @pytest.mark.parametrize("curve_name,hash_algo", [
    ...
    def test_ecdsa_signature_compliance(self, curve_name, hash_algo):
        ...
        for i, message in enumerate(test_messages):
            # Sign message
            signature = crypto.brainpool_sign(message, private_key, hash_algorithm=hash_algo)
            
            # Validate signature format
            is_valid, issues = validator.validate_ecdsa_signature(curve_name, signature, hash_algo)
            assert is_valid, \
                f"Signature {i+1} failed BSI TR-03111 validation: {', '.join(issues)}"
            
            # Verify signature
            verification_result = crypto.brainpool_verify(message, signature, public_key, hash_algorithm=hash_algo)
            assert verification_result, \
                f"Signature {i+1} verification failed"
    ...
    def test_approved_curves_only(self):
        ...
        for curve in approved_curves:
            assert validator.validate_curve_parameters(curve), \
    ...
    def test_key_serialization_compliance(self, curve_name):
        ...
        assert public_pem is not None and len(public_pem) > 0, \
            "Public key serialization failed"
        assert private_pem is not None and len(private_pem) > 0, \
        ...
        signature = crypto.brainpool_sign(test_message, loaded_private, hash_algorithm='sha256')
        verification = crypto.brainpool_verify(test_message, signature, loaded_public, hash_algorithm='sha256')
    ...
    def test_security_level_requirements(self):
        ...
        assert validator.APPROVED_CURVES['brainpoolP256r1']['security_level'] >= 128, \
            "brainpoolP256r1 must provide at least 128-bit security"
        assert validator.APPROVED_CURVES['brainpoolP384r1']['security_level'] >= 192, \
            "brainpoolP384r1 must provide at least 192-bit security"
        assert validator.APPROVED_CURVES['brainpoolP512r1']['security_level'] >= 256, \
