...
class RFC7027Parser:
    ...
    def parse_file(file_path: str) -> List[RFC7027TestVector]:
        ...
        for line in lines:
            ...
            if all(k in current_vector for k in ['curve_name', 'private_key', 'public_key']):
                vectors.append(RFC7027TestVector(
    ...
    def _parse_json_item(item: Dict) -> Optional[RFC7027TestVector]:
        ...
        try:
            ...
            if 'note' in item or (not item.get('curve') and not item.get('private_key')):
                return None
            ...
            return RFC7027TestVector(
                ...
                signature=bytes.fromhex(item.get('signature', '')) if item.get('signature') else None,
...
class RFC6954Parser:
        ...
        for line in lines:
            ...
            if 'initiator' in line.lower():
                if 'private' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{64,})', line)
                    if hex_match:
                        current_vector['initiator_private'] = bytes.fromhex(hex_match.group(1))
                elif 'public' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{128,})', line)
                    if hex_match:
                        current_vector['initiator_public'] = bytes.fromhex(hex_match.group(1))
            
            if 'responder' in line.lower():
                if 'private' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{64,})', line)
                    if hex_match:
                        current_vector['responder_private'] = bytes.fromhex(hex_match.group(1))
                elif 'public' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{128,})', line)
                    if hex_match:
                        current_vector['responder_public'] = bytes.fromhex(hex_match.group(1))
            ...
            required = ['curve_name', 'initiator_private', 'initiator_public',
                       'responder_private', 'responder_public', 'shared_secret']
            if all(k in current_vector for k in required):
                vectors.append(RFC6954TestVector(
                    curve_name=current_vector['curve_name'],
    ...
    def _parse_json_item(item: Dict) -> Optional[RFC6954TestVector]:
        ...
        try:
            # Skip placeholder entries
            if 'note' in item or (not item.get('curve') and not item.get('initiator_private')):
                return None
...
class RFC8734Parser:
        ...
        for line in lines:
            ...
            if 'client' in line.lower():
                if 'private' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{64,})', line)
                    if hex_match:
                        current_vector['client_private'] = bytes.fromhex(hex_match.group(1))
                elif 'public' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{128,})', line)
                    if hex_match:
                        current_vector['client_public'] = bytes.fromhex(hex_match.group(1))
            
            if 'server' in line.lower():
                if 'private' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{64,})', line)
                    if hex_match:
                        current_vector['server_private'] = bytes.fromhex(hex_match.group(1))
                elif 'public' in line.lower():
                    hex_match = re.search(r'([0-9a-fA-F]{128,})', line)
                    if hex_match:
                        current_vector['server_public'] = bytes.fromhex(hex_match.group(1))
            ...
            # Create vector when we have all required fields
            required = ['curve_name', 'client_private', 'client_public',
                       'server_private', 'server_public', 'shared_secret']
            if all(k in current_vector for k in required):
                vectors.append(RFC8734TestVector(
                    curve_name=current_vector['curve_name'],
    ...
    def _parse_json_item(item: Dict) -> Optional[RFC8734TestVector]:
        ...
        try:
            # Skip placeholder entries
            if 'note' in item or (not item.get('curve') and not item.get('client_private')):
                return None
            ...
            return RFC8734TestVector(
                ...
                shared_secret=bytes.fromhex(item.get('shared_secret', '')),
                handshake_hash=bytes.fromhex(item.get('handshake_hash', '')) if item.get('handshake_hash') else None,
                comment=item.get('comment', '')
            )
        except (KeyError, ValueError):
            return None
