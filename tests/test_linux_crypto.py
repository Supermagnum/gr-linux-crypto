...
AUTH_MODES = {'aes-128': ['gcm', None], 'aes-256': ['gcm', None], 'chacha20': ['poly1305']}
...
class TestEncryptDecryptRoundTrip:
    ...
    def test_aes_128_round_trip(self, algorithm, size, random_key_128):
        ...
        ciphertext, iv, auth_tag = encrypt(algorithm, random_key_128, data, iv_mode="random", auth='gcm')
        decrypted = decrypt(algorithm, random_key_128, ciphertext, iv, auth='gcm', auth_tag=auth_tag)
    ...
    def test_aes_256_round_trip(self, algorithm, size, random_key_256):
        ...
        data = secrets.token_bytes(size) if size > 0 else b""
        ciphertext, iv, auth_tag = encrypt(algorithm, random_key_256, data, iv_mode="random", auth='gcm')
        decrypted = decrypt(algorithm, random_key_256, ciphertext, iv, auth='gcm', auth_tag=auth_tag)
    ...
    def test_chacha20_poly1305_round_trip(self, size, random_key_chacha20):
        ...
        data = secrets.token_bytes(size) if size > 0 else b""
        ciphertext, nonce, auth_tag = encrypt('chacha20', random_key_chacha20, data, 
                                             iv_mode="random", auth='poly1305')
        decrypted = decrypt('chacha20', random_key_chacha20, ciphertext, nonce, 
                           auth='poly1305', auth_tag=auth_tag)
    ...
    def test_non_authenticated_round_trip(self, algorithm, auth, variable_size_data):
        ...
        key = secrets.token_bytes(16) if algorithm == 'aes-128' else secrets.token_bytes(32)
        
        # For CBC mode, we need fixed IV for reproducible tests
        iv = secrets.token_bytes(16)
        ciphertext, returned_iv, auth_tag = encrypt(algorithm, key, variable_size_data, 
                                                     iv_mode=iv, auth=auth)
        assert returned_iv == iv, "IV should match provided IV"
        assert auth_tag is None, "Non-authenticated mode should return None for auth_tag"
        
        decrypted = decrypt(algorithm, key, ciphertext, returned_iv, auth=auth, auth_tag=auth_tag)
...
class TestDeterminism:
    ...
    def test_aes_gcm_determinism_with_fixed_iv(self, test_data_small, random_key_128):
        ...
        iv = b'\x01' * 12
        ciphertext1, iv1, tag1 = encrypt('aes-128', random_key_128, test_data_small, 
                                         iv_mode=iv, auth='gcm')
        ciphertext2, iv2, tag2 = encrypt('aes-128', random_key_128, test_data_small, 
                                         iv_mode=iv, auth='gcm')
        
        assert ciphertext1 == ciphertext2, "Same plaintext + key + IV should produce same ciphertext"
    ...
    def test_chacha20_poly1305_determinism(self, test_data_small, random_key_chacha20):
        ...
        nonce = b'\x02' * 12
        ciphertext1, nonce1, tag1 = encrypt('chacha20', random_key_chacha20, test_data_small, 
                                            iv_mode=nonce, auth='poly1305')
        ciphertext2, nonce2, tag2 = encrypt('chacha20', random_key_chacha20, test_data_small, 
                                            iv_mode=nonce, auth='poly1305')
        
        assert ciphertext1 == ciphertext2, "Same plaintext + key + nonce should produce same ciphertext"
...
class TestKeyUniqueness:
    ...
    def test_different_keys_produce_different_ciphertexts(self, test_data_small):
        ...
        iv = b'\x03' * 12
        ciphertext1, iv1, tag1 = encrypt('aes-128', key1, test_data_small, iv_mode=iv, auth='gcm')
        ciphertext2, iv2, tag2 = encrypt('aes-128', key2, test_data_small, iv_mode=iv, auth='gcm')
        
        assert ciphertext1 != ciphertext2, "Different keys should produce different ciphertexts"
        assert tag1 != tag2, "Different keys should produce different auth tags"

    def test_same_key_different_data_produces_different_ciphertexts(self, random_key_128):
        """Test that same key with different data produces different ciphertexts."""
        data1 = b"Message 1"
        data2 = b"Message 2"
        
        iv = b'\x04' * 12
        ciphertext1, _, _ = encrypt('aes-128', random_key_128, data1, iv_mode=iv, auth='gcm')
        ciphertext2, _, _ = encrypt('aes-128', random_key_128, data2, iv_mode=iv, auth='gcm')
        
        assert ciphertext1 != ciphertext2, "Different data should produce different ciphertexts"
...
class TestErrorHandling:
    ...
    def test_missing_auth_tag_gcm(self, random_key_128):
        ...
        with pytest.raises(ValueError, match="GCM mode requires auth_tag"):
            decrypt('aes-128', random_key_128, ciphertext, iv, auth='gcm', auth_tag=None)

    def test_missing_auth_tag_poly1305(self, random_key_chacha20):
        """Test error handling when auth tag is missing for Poly1305."""
        data = b"test data"
        ciphertext, nonce, auth_tag = encrypt('chacha20', random_key_chacha20, data, auth='poly1305')
        
        with pytest.raises(ValueError, match="Poly1305 authentication requires auth_tag"):
            decrypt('chacha20', random_key_chacha20, ciphertext, nonce, auth='poly1305', auth_tag=None)
    ...
    def test_corrupted_auth_tag_gcm(self, random_key_128):
        ...
        with pytest.raises(ValueError, match="GCM authentication failed"):
            decrypt('aes-128', random_key_128, ciphertext, iv, auth='gcm', auth_tag=corrupted_tag)
    ...
    def test_corrupted_auth_tag_poly1305(self, random_key_chacha20):
        ...
        with pytest.raises(ValueError, match="Poly1305 authentication failed"):
            decrypt('chacha20', random_key_chacha20, ciphertext, nonce, 
                   auth='poly1305', auth_tag=corrupted_tag)
    ...
    def test_corrupted_ciphertext_gcm(self, random_key_128):
        ...
        with pytest.raises(ValueError, match="GCM authentication failed"):
            decrypt('aes-128', random_key_128, corrupted_ciphertext, iv, 
                   auth='gcm', auth_tag=auth_tag)
    ...
    def test_chacha20_without_poly1305(self, random_key_chacha20):
        ...
        with pytest.raises(ValueError, match="ChaCha20 requires Poly1305 authentication"):
...
class TestPerformance:
    ...
    def test_encryption_performance_aes128_gcm(self, random_key_128):
        ...
        assert time_per_block_us < PERFORMANCE_THRESHOLD_US, \
    ...
    def test_decryption_performance_aes128_gcm(self, random_key_128):
        ...
        # Warm up
        for _ in range(10):
            decrypt('aes-128', random_key_128, ciphertext, iv, auth='gcm', auth_tag=auth_tag)
...
class TestOpenSSLCrossValidation:
    ...
    def test_gr_encrypt_openssl_decrypt(self, algorithm, auth):
        ...
        ciphertext, iv, auth_tag = encrypt(algorithm, key, data, iv_mode="random", auth=auth)
    ...
    def test_random_cross_validation(self, algorithm, auth, iteration):
        ...
        decrypted = decrypt(algorithm, key, ciphertext, iv, auth=auth, auth_tag=auth_tag)
        
        assert decrypted == data, f"Round-trip failed on iteration {iteration}"
        assert len(ciphertext) == len(data) or len(ciphertext) >= len(data), \
