...
class blk(gr.sync_block):
    def __init__(self):
        ...
        self._frame_size_with_signature = 8 + 64  # Codec2 frame (8) + signature (64) = 72 bytes
    ...
    def work(self, input_items, output_items):
        ...
        if len(self._signed_buffer) >= self._frame_size_with_signature and len(self._key_buffer) >= 32:
            try:
                signed_frame = bytes(self._signed_buffer[:self._frame_size_with_signature])
                self._signed_buffer = self._signed_buffer[self._frame_size_with_signature:]
                ...
                if NACL_AVAILABLE and len(public_key) == 32 and len(signature) == 64:
                    try:
                        is_valid = nacl.verify_ed25519(codec2_frame, signature, public_key)
                        if is_valid:
                            ...
                            print(f"FAILED: FreeDV Codec2 frame signature is INVALID")
                        ...
                        print("Warning: gr-nacl verify_ed25519 not found. Cannot verify.")
                        out_data = np.frombuffer(codec2_frame[:out_len], dtype=np.uint8)
                        if len(out_data) <= len(output_items[0]):
                            output_items[0][:len(out_data)] = out_data
                        return len(out_data)
                else:
                    print("Warning: Verification not available - missing key or signature")
                    out_data = np.frombuffer(codec2_frame[:out_len], dtype=np.uint8)
                    if len(out_data) <= len(output_items[0]):
                        output_items[0][:len(out_data)] = out_data
                    return len(out_data)
