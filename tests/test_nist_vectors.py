...
try:
    from test_vectors import (
        NISTCAVPParser, RFC8439Parser,
        AESGCMTestVector, ChaCha20Poly1305TestVector
    )
except ImportError:
    from .test_vectors import (
        NISTCAVPParser, RFC8439Parser,
...
def test_nist_aes_gcm_128_vectors():
    ...
    if not NIST_AES_GCM_128_FILE.exists():
        pytest.skip(f"NIST AES-GCM-128 test vectors not found at {NIST_AES_GCM_128_FILE}")
            ...
            try:
                ...
                if decrypted != vector.plaintext:
                    results.add_result(
                        False,
                        f"Vector {vector.count}",
                        f"Decryption failed: plaintext mismatch"
                    )
                    continue
    ...
    # Assert all passed
    assert results.failed == 0, f"{results.failed} test vectors failed. See details above."
...
def test_nist_aes_gcm_256_vectors():
    ...
    if not NIST_AES_GCM_256_FILE.exists():
        pytest.skip(f"NIST AES-GCM-256 test vectors not found at {NIST_AES_GCM_256_FILE}")
...
def test_rfc8439_chacha20_poly1305_vectors():
    ...
    if not RFC8439_CHACHA20_FILE.exists():
        pytest.skip(f"RFC 8439 ChaCha20-Poly1305 test vectors not found at {RFC8439_CHACHA20_FILE}")
...
def create_sample_test_vectors(tmp_path_factory):
    ...
    aes_gcm_sample.write_text("""Count = 0
